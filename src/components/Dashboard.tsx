...
import { PlusCircle, Copy, ExternalLink, BarChart2, Settings, Trash2, Clock, CheckCircle, Code } from 'lucide-react';
...
export const Dashboard = () => {
  ...
  const { forms, fetchForms, publishForm, unpublishForm, deleteForm, loading: formsLoading } = useFormStore();
      ...
      if (!authLoading && user) {
        try {
          ...
          for (const form of forms) {
            const submissionsSnapshot = await getDocs(collection(db, `forms/${form.id}/submissions`));
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {forms.length === 0 ? (
          <div className="bg-white rounded-xl shadow-sm p-12 text-center">
            ...
            <p className="text-gray-500 mb-8">Get started by creating a new form to collect responses</p>
            <Link
              to="/builder"
              className="inline-flex items-center px-6 py-3 border border-transparent text-base font-medium rounded-lg shadow-sm text-white bg-blue-600 hover:bg-blue-700 transition-colors"
            >
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
            {forms.map((form) => (
              <div key={form.id} className="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100 hover:border-blue-500 transition-colors">
                <div className="p-6">
                  ...
                  <div className="border-t border-gray-100 bg-gray-50 p-4">
                    <div className="space-y-2">
                      ...
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="text-gray-900">{submissionCounts[form.id] || 0}</span>
                      </div>
                    </div>
                  </div>
                )}
